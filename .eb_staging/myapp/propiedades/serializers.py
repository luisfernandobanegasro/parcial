...
class UsuarioUnidadSerializer(serializers.ModelSerializer):
    # Extras sólo lectura para la UI
    ...
    class Meta:
        ...
        fields = [
            ...
            # extras
            "usuario_nombre", "usuario_correo", "unidad_codigo",
            "condominio_id", "condominio_nombre", "activo",
        ]
...
class UnidadListSerializer(serializers.ModelSerializer):
    """
    Serializer para listados con campos ya “resueltos” que la UI necesita.
    """
    ...
    propietario_nombre  = serializers.CharField(read_only=True, allow_blank=True)
    activo              = serializers.SerializerMethodField()

    class Meta:
        model  = Unidad
        fields = (
            "id", "codigo", "piso", "area_m2",
    ...
    def get_activo(self, obj):
        # Considera “activo” si estado == 'activo' (case-insensitive)
        return (obj.estado or "").lower() == "activo"
