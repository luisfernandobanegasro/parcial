...
def render_recibo_pdf_bytes(request, contexto):
    ...
    try:
        ...
        try:
            # Importar dinámicamente para evitar que analizadores estáticos marquen
            # el import como error cuando reportlab no está instalado en el entorno.
            import importlib
            ...
            # Normalizar nombres que se usan más abajo (útil para linters/editores)
            Canvas = getattr(canvas_module, "Canvas", None)
            if Canvas is None:
                raise RuntimeError("ReportLab: no se encontró Canvas")
            # mantenemos el módulo en `canvas` y los valores A4/mm ya están definidos arriba
            canvas = canvas_module
        buff = BytesIO()
